...
export function AdminChart() {
  ...
  const generateChartData = () => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const baseUsers = userStats?.length || 0
      const baseEnrollments = courseStats?.reduce((sum: number, c: any) => 
        sum + (c.totalEnrollments || 0), 0) || 0
      
      data.push({
        date: date.toISOString().split("T")[0],
        users: Math.floor(baseUsers * (0.7 + Math.random() * 0.3)),
        enrollments: Math.floor(baseEnrollments * (0.7 + Math.random() * 0.3)),
      })
    }
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>User Activity & Enrollments</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            Track user activity and enrollment trends
          </span>
